...
def detect_signal(interval_check, result: dict) -> tuple:
    ...
    if kline_data is None or len(kline_data) < Config.KLINE_LIMIT and cf.INTERVAL_TO_MIN.get(interval_check) < 240:
        return (0, None)
    if kline_data is None or len(kline_data) < 5 and cf.INTERVAL_TO_MIN.get(interval_check) >= 240:
        return (0, None)
    ...
    if 'LONG' in Config.POSITION_SIDE:
        if interval_check == '1h':
            ...
            if (red_green and price_power(1.1) and rsi(3) < 40) or (
                    structure.is_morning_star(kline_data) and rsi(4) <= 30) or (red_green and price_power(2.5)):
                has_signal = (1, '做多')
    ...
    if 'SHORT' in Config.POSITION_SIDE:
        if interval_check == '1h':
            ...
            # ema12 = ema_atr.calculate_ema(prices=close_prices, period=12)[-2]
            red_green = (latest['close'] < latest['open']) and (prev['close'] > prev['open'])
            if (red_green and price_power(1.1) and rsi(3) > 75):
                has_signal = (-1, '做空')
    return has_signal
